...
- **Networking** — VPC with public and private subnet CIDRs provisioned across 2 AZs, security groups scoped per tier (ALB, ECS tasks, data tier). ALB, ECS, and the Postgres instance currently run in the **public subnets** — the private subnets exist but have no NAT gateway or private route table wired up yet, so they're not reachable/usable as-is (this is called out directly in the architecture diagram below)
...
<img src="docs/architecture-animated.svg" alt="Animated architecture diagram showing infrastructure provisioning to S3 remote state, the application CI/CD deploy flow, and live request traffic flow through the VPC" width="100%">
...
- **Remote state** is stored in a single encrypted S3 bucket, keyed per-environment (`${path_relative_to_include()}/terraform.tfstate`). State locking relies on S3's native locking support — there's no DynamoDB lock table in this setup.
